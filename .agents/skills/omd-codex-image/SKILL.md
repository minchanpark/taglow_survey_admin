---
name: omd-antigravity-image
description: "이미지 placeholder를 동적으로 materialize. Antigravity 채널에서는 내장 `generate_image` 툴 호출, Antigravity 채널에서는 omd-asset-curator로 fall back, OpenCode에서는 spec dump. HTML/MD의 `<!-- omd:gen-image -->` 블록을 단일 source of truth로 사용. '이미지 생성해줘', '플레이스홀더 채워줘', '안티그래비티로 이미지 만들어' 류 트리거."
---

# omd:antigravity-image

채널-aware 이미지 materialization 스킬. **하나의 spec format** + **세 가지 downstream 처리**.

문제: oh-my-design이 emit하는 HTML/MD에는 늘 illustration 자리가 비어있다. 채널별 capability가 다르다 — Antigravity CLI는 내장 image generation을 갖고, Antigravity는 안 가졌고, OpenCode는 user에게 위임한다. 이 차이를 skill 1개로 통합한다.
...
### 1.1 channel = `antigravity`

Antigravity는 native image generation 도구인 `generate_image`를 가지고 있다. 다음 순서:

1. artifact를 Read로 전체 로드
2. `<!-- omd:gen-image ... -->` 블록을 정규식으로 추출 (multiline)
3. 각 블록마다 Antigravity의 `generate_image` 툴을 호출하여 이미지를 생성한다. `Prompt`와 `ImageName` 파라미터를 사용한다.
4. 출력 파일을 `filename` 경로에 저장 (mkdir -p)
5. **주석 블록은 그대로 두고** `<img src>`나 `![]()` 경로가 매칭되는지 확인. 안 맞으면 alt 텍스트 보존하면서 src만 교체.
6. 각 처리 항목에 `<!-- omd:gen-image:done at=<ISO timestamp> by=antigravity -->` 주석을 spec 블록 아래에 1줄 추가.

generation 실패 시: spec 블록은 그대로, `<!-- omd:gen-image:error reason="..." -->` 추가. 다음 호출이 재시도 가능하도록.

prompt 보강 (Antigravity `generate_image`가 좋은 결과를 내려면):
...
### 1.2 fallback (or channel = `antigravity` without tool access)

Antigravity의 `generate_image` 툴을 사용할 수 없거나 실패한 경우, `omd-asset-curator` 스킬로 라우팅하여 처리합니다:
...
5. **사용자에게 1줄 알림**: "Antigravity에서는 free-license fallback을 적용했어요. 더 정확한 이미지는 Antigravity 채널에서 generate하면 돼요."
...
- 브랜드 로고는 generation이 아니라 reference에서 **다운로드**해서 가져오기. Antigravity/asset-curator 둘 다 로고는 generate 금지 (왜곡된 가짜 로고는 IP risk).
...
```
✓ 4 images materialized (antigravity)  ·  0 skipped  ·  0 errors
...
```
✓ 2 antigravity · 2 asset-curator fallback · 1 user-queue
```

## 6. 호출 envelope

```yaml
agent: omd-antigravity-image
inputs:
  artifact_path: experiments/2026-05-19/karrot/landing.html
  channel: antigravity          # 또는 antigravity | opencode | auto
...
- Antigravity `generate_image`가 사용 불가한 환경에서 channel=antigravity 강제 호출 X — auto 또는 명시적 fallback 사용.
- 같은 filename에 다른 prompt 두 번 처리 금지 (idempotency violation).
- generation prompt에 사람 신원이나 실명 절대 넣지 않기.

## 8. 다른 스킬과의 관계

- **omd-asset-curator**: 무료 라이선스 자산 카탈로그. antigravity-image의 antigravity fallback path.
- **omd-reference-capture**: 라이브 브랜드 사이트 캡쳐. `references:` URL 검증용으로 같이 쓰면 정확도 ↑.
- **omd-orchestrator**: HTML emit 후 antigravity-image를 마지막 단계에 끼워 넣을 수 있음. orchestrator의 worker 카탈로그에 추가됨.
