...
```sh
rg -n "테스트 전략|Database Policy Test|구현 순서|Error Handling|완료 기준" dev/Taglow_survey_Admin_TDD_v2.md dev/Taglow_Survey_Admin_PRD.md
```
...
- Auth: unauthenticated redirect, wrong domain denied, missing/inactive `admin_members` denied, active admin enters.
...
For independent final review, use `.agents/agents/taglow-admin-test-qa-auditor.md`. Keep it read-only.
For performance-specific final review, use `.agents/agents/taglow-admin-performance-auditor.md`. Keep it read-only.
