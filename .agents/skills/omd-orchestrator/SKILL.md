...
description: "멀티 에이전트 디자인 워크플로우 supervisor. omd-kr-writer, omd-locale-adapter, omd-designer-review, omd-final-qa, omd-antigravity-image를 routing. 2-round revision cap 유지. 블로그/컴포넌트/아티클 등 multi-role 작업에 트리거 ('블로그 글 작성', 'KR+EN 동시 발행', '디자인 리뷰 받고 다듬어줘')."
---
...
> 런타임 dependency 없음. Antigravity의 subagent 호출 메커니즘이 그대로 orchestrator-workers 토폴로지를 구현한다.
...
| Image materializer | `omd-antigravity-image` | `<!-- omd:gen-image -->` 블록을 채널별로 실체화 (Antigravity native gen / asset-curator fallback / OpenCode user-queue) |
...
Stage 4i IMAGES      omd-antigravity-image  (`<!-- omd:gen-image -->` 블록 처리, 채널별 분기)
...
Antigravity subagent 호출 시 다음 envelope:
