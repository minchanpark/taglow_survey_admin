...
```
omd-master subagent가 이 세션에 등록되어 있지 않아요. Antigravity는 세션 시작 시점에만 .agents/agents/*.md를 로드합니다 — install-skills를 세션 띄운 후에 돌렸으면 이 케이스.

해결 (가장 빠른 순서):
1. 현재 세션에서 define_subagent/invoke_subagent 툴 실행 — Antigravity가 .agents/agents/*.md 강제 재스캔. omd-* 8개가 목록에 나타나면 /omd-harness 재호출.
2. 안 되면: Antigravity 앱 완전 종료 (Cmd+Q / 터미널 자체 quit) → 새 터미널 → Antigravity 재실행 → /omd-harness <task> 재호출.
```
...
9. **Hero archetype selection** (v1.3.8 신설 — monotone hero 회귀 방지):
   ...
   **G. `quote-led`**
   ...
   | AI / Developer tools — anthropic, antigravity, vercel | B `center-text` | F `dashboard-preview` |
