...
create policy "admin can update own surveys"
on public.surveys
for update
to authenticated
...
create policy "admin can delete own draft surveys"
on public.surveys
for delete
to authenticated
using (private.is_admin_editor() and created_by = auth.uid() and status = 'draft');

drop policy if exists "admin can manage sections of own surveys" on public.survey_sections;
create policy "admin can manage sections of own surveys"
...
create policy "admin can manage questions of own surveys"
on public.questions
for all
to authenticated
using (
...
create policy "admin can manage assets of own surveys"
...
for all
to authenticated
using (bucket_id = 'survey-assets' and private.is_admin_editor())
with check (bucket_id = 'survey-assets' and private.is_admin_editor());
